...
# Some unused code - interesting to keep
...
#' For IS files, make two new columns for horizontal position (HOR) and vertical position (VER)
#'
#' @param df A data frame of data
#' @param splitName The name of the corresponding data file, split by period into a list of component parts
#' @return a data frame with the new columns
#' @export
assignHorVer <- function(df, splitName){
  df$horizontalPosition <- as.character(rep(splitName[[1]][8], nrow(df)))
  df$verticalPosition <- as.character(rep(splitName[[1]][9], nrow(df)))
  return(df)
}
...
#Would like to get back to putting HOR, VER in columns, see below for function.
...
#' @export
assignHorVer <- function(df, splitName){
  df$horizontalPosition <- rep(as.character(splitName[[1]][8]), nrow(df))
  df$verticalPosition <- rep(as.character(splitName[[1]][9]), nrow(df))
  df$horizontalPosition <- as.character(df$horizontalPosition)
  df$verticalPosition <- as.character(df$verticalPosition)
  return(df)
}
...
stackDataFiles <- function(folder){
  ...
  if(length(datafls)>1){                                  # if there is more than one data file
    ...
    for(i in 1:length(tables)){
      ...
      if(length(d.splitName[[1]]) %in% c(12,14)){d <- assignHorVer(d, d.splitName)}
      ...
      if(length(f) > 1){
        for(j in 2:length(f)){
          ...
          if(length(d.next.splitName[[1]]) %in% c(12,14)){d.next <- assignHorVer(d.next, d.next.splitName)}
