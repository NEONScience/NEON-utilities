...
zipsByProduct <- function(dpID, site="all", package="basic", check.size=T) {
  ...
  productUrl <- paste("http://data.neonscience.org/api/v0/products/", dpID, sep="")
  req <- httr::GET(productUrl)
  avail <- jsonlite::fromJSON(httr::content(req, as="text"), simplifyDataFrame=T, flatten=T)
